// ============================================================
// npcs.js — Единорожки и бабочки (v2)
//
// Фиксы:
// - Единорожки красивее (грива пышнее, хвост радужный, копытца)
// - Приручённые ходят на разных дистанциях (не слипаются)
// - Можно приручать всех единорожков
// ============================================================
...
const NPCs = {
    ...
    _createUnicorn(x, z) {
        ...
        // Belly (нижняя часть чуть розовая)
        ...
        // Horn — золотой, конусообразный (3 секции)
        ...
        g.userData = {
            tamed: false,
            tamedIndex: -1, // порядковый номер приручения (для разнесения)
            aiTimer: 0,
    ...
    update(dt, t) {
        ...
        this.unicorns.forEach(u => {
            const d = u.userData;
            if (d.tamed) {
                // Каждый приручённый идёт на своём расстоянии и под своим углом
                // чтобы не слипались
                const followDist = 3.5 + d.tamedIndex * 2.5;
                const angleOffset = d.tamedIndex * 1.2; // разный угол отставания

                // Точка назначения — позади Тёпы со смещением
                // Forward = (sin, cos), Behind = (-sin, -cos)
                ...
                if (dist > 1) {
                    const speed = Math.min(dist * 2, 5); // быстрее если далеко
